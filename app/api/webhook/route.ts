  ...
  if (session?.step === "WAITING_DESTINATION") {
    ...
    if (msg.type === "location") {
      ...
      if (!geocoded) {
        await lineClient.replyMessage({
          replyToken,
          messages: [{ type: "text", text: `ไม่พบสถานที่ "${text}" ลองพิมพ์ใหม่หรือส่ง location ปลายทางแทนครับ` }],
        });
        return;
      }
    ...
    if (routes.length === 0) {
      ...
      await lineClient.replyMessage({
        replyToken,
        messages: [{ type: "text", text: "ขอโทษครับ ไม่พบเส้นทางขนส่งสาธารณะหรือทางเลือกสีเขียวในพื้นที่นี้" }],
      });
      return;
    }
